...
4.  **Phase Mismatch (Δβ) vs. Pump Frequency Plot**: Shows the phase mismatch as a function of the pump frequency and helps when make changes in the dispersion engineering.
5.  **Output Power vs Frequency**: Plots of Each Harmonic (1 - 9) at a given frequency and their correpsonding power.
6.  **Output Power vs Position**: Snapshot of the device and how the output power of each harmonic (1 - 9) changes with position.
...
*   **`analyze_delta_beta.m`**
    ...
    *   **Usage**: Run this after generating dispersion data to check if your device geometry supports the desired nonlinear process. The goal is to engineer the dispersion to make `Δβ ≈ 0` over a wide bandwidth.
    *   **Output**: A plot of `Δβ` vs. frequency. Regions where `Δβ` is close to zero are where efficient parametric processes can occur.

---
## 2. Getting Started: Running Your First Simulation

Follow these steps to run a basic simulation and generate the primary device characteristics.

1.  **Open MATLAB** and navigate to the `TWPAv2` project directory.
2.  **Open the Recommended Script**: Open the file `network_calc_adaptive_freq.m`. This script is the best starting point as it uses an efficient adaptive frequency grid.
3.  **Review Parameters**: At the top of the script, you will find sections for defining the device's physical parameters (e.g., `g.finger.l`, `g.ms.h`, `g.finger.modamp`). For your first run, you can use the default values.
4.  **Run the Simulation**: Execute the script by pressing F5 or typing `network_calc_adaptive_freq` in the MATLAB command window. The simulation may take several minutes depending on the complexity and frequency range.
5.  **Analyze the Output**: Upon completion, two figures will be generated:
    *   **S21 vs. Frequency**: Shows the transmission magnitude through the device.
    *   **Dispersion (k vs. Frequency)**: Shows the propagation constant, which is critical for phase-matching analysis.
6.  **Find the Saved Data**: The script also saves the key results (frequency array `f`, S21 data, and dispersion `kperm`) into a `.mat` file (e.g., `0714TWPaX.mat`). This data is the input for all further harmonic and advanced analysis.

---

## 3. Simulation Workflow: From Physics to Code
...
## 2.4. Harmonic Simulation & Analysis Scripts
...
**`simulate0714harmonics_frequency_optismation.m`**
*   **Purpose**: Sweeps the pump frequency and simulates the generation of odd harmonics (1st, 3rd, 5th, etc.) in the TWPA using the coupled mode equations.
*   **Inputs**: Requires a `.mat` file with device parameters (`f`, `kperm`, `S21`).
*   **Outputs**: Plots harmonic power vs. pump frequency, saves figures, and outputs arrays of end currents for each harmonic. These results are used to optimize device design for specific harmonics.
*   **Usage**: Run this script after generating the device data. It is the main entry point for harmonic simulation.
...
*   **Purpose**: Calculates and visualizes phase mismatch (Δβ) for selected harmonics as a function of pump frequency, and compares phase matching for different harmonics.
*   **Inputs**: Device `.mat` file, simulation parameters.
*   **Outputs**: Plots Δβ vs. pump frequency for each harmonic, and scatter plots comparing phase mismatch between harmonics.
*   **Usage**: Use this script to guide device design for optimal phase matching and efficient harmonic generation.

### 2.5. Parameter Sweep Scripts
