...
% Define pump frequency sweep range to determine harmonic limit
fcalc = 0.1e9:0.01e9:4.3e9;
...
twpa.modes = [1 0]; % set up for harmonic modes (e.g. 1p, 3p, ..., 9p)
...
% --- Find optimal frequency for the desired harmonic and prepare plot titles ---
mode_to_optimize_idx = (harmonic_to_optimize_for + 1) / 2;

% Initialize return values and plot title
optimal_pump_freq_GHz = NaN;
max_power_dB = NaN;

% Check if this harmonic was actually simulated
if mode_to_optimize_idx > size(g_data, 3) || mod(harmonic_to_optimize_for, 2) == 0 || harmonic_to_optimize_for < 1
    ...
    plot_freq_for_title = fcalc(optimal_freq_idx) / 1e9;
    plot_title_text = sprintf('Power vs Position at Last Freq (%.2f GHz)', plot_freq_for_title);
else
    % Find the frequency index that has the maximum power for the chosen harmonic
    ...
    fprintf('Optimized for %d-th harmonic. Max power: %.2f dB at pump freq: %.3f GHz\n', ...
        harmonic_to_optimize_for, max_power_dB, optimal_pump_freq_GHz);
...
% Create a larger figure to accommodate 4 plots, but keep it hidden for speed
...
% Subplot 2 (Top-Right): Dispersion vs Frequency
subplot(2, 2, 2);
...
% Subplot 3 (Bottom-Left): Output Power vs Frequency
subplot(2, 2, 3);
hold all;
plotLegend = {};
...
legend(plotLegend, 'Location', 'best'); grid on; xlim([fcalc(1) fcalc(end)]./1e9); ylim([-30 5]); xlabel('Frequency (GHz)'); ylabel('Output Power (dB)'); title('Harmonic Output Power vs. Frequency'); set(gca,'FontSize',10,'FontWeight','bold');

% Subplot 4 (Bottom-Right): Output Power vs Position
subplot(2, 2, 4);
hold all;
for h_idx = 1:ceil(maxHarmonic/2)
    plot(zcalc./twpa.len, smooth(g_data(optimal_freq_idx,:,h_idx),1), 'Linewidth', 2);
end
